                ...
                for k in self.source_item_claim_evi[i][j].keys():
                    for l in self.item_val_dis[j].keys():
                        ...
                        if l == k:
                            ...
                            if self.item_val_dis[j][l] == -1:
                                self.item_val_dis[j][l] = float(1 - self.Accuracy[i] * expit(self.Extractor[j]['tfidf'] * cos1 + self.Extractor[j]['glove'] * cos2))/float(self.total_claim)
                            else:
                                self.item_val_dis[j][l] = self.item_val_dis[j][l] * float(1 - self.Accuracy[i] * expit(self.Extractor[j]['tfidf'] * cos1 + self.Extractor[j]['glove'] * cos2)) / float(self.total_claim)
        # Normalization
        for i in self.item_val_dis.keys(): # item_id
            sum_total = 0
            for j in self.item_val_dis[i].keys():
